...
class Booking_Boat_Rental_Woocommerce_Public {
	...
	public function boat_shop_filter(){
		...
		foreach($pricing as $val){
			if($val->slug == $price_type){
				$type = $val->name;
			}
		}
		$pricing_key = 'bbrw_pricing_val_'.$price_type;
...
<?php	}
	...
	function ipe_product_custom_price( $cart_item_data, $product_id ) {
		...
		if(isset($_POST['capacity']) && !empty($_POST['capacity'][1])){
			$cart_item_data[ "capacity" ] = $_POST['capacity'][1];
		}
	...
	public function boat_add_field_checkout( $fields ) {
		...
		$fields['billing_birthday'] = array(
	        ...
	        'required'    => true,
	        'clear'       => true,
	        'custom_attributes' => array( 'extra-checkout-birthday-description'=>"You must be at least 18 years old to sign up. Other users won't see your birthday." ),
		...
		if(!empty($items)){ 
			...
			?>
				<div class="reservation-information">
					...
					<div class="item">Capacity: <?= !empty($val['capacity'])? $val['capacity']:'--'?> </div>
					...
					<div class="c-coupons">
						<div><strong>Coupons Applied</strong></div>
						<div>Code: <span><b><?= (WC()->cart->applied_coupons)[0]; ?></b></span></div>
						<div>Discount: <?= get_woocommerce_currency_symbol().WC()->cart->get_discount_total(); ?></div>
						<div>Total: <?= $woocommerce->cart->get_cart_total()?></div>
					</div>
					<?php endif; ?>
				</div>
				<h2 id="product_name_review_heading"><a class="b-link" href="<?php echo get_the_permalink($val['data']->get_id()); ?>"><?= $_product->get_title('woocommerce_thumbnail'); ?></a></h2>
				<div class="product-information">
					<div class="thumb"><?php if(!empty($image)): ?>
					<a class="b-inline" href="<?php echo get_the_permalink($val['data']->get_id()); ?>">
						<img src="<?= $image[0]; ?>"/>
					</a>
					<?php endif; ?>
					</div>
	...
	public function boat_save_custom_field_added_checkout( $order_id ){
		...
		if( !empty( $_POST['capacity'] ) )
			update_post_meta( $order_id, 'capacity', sanitize_text_field( $_POST['capacity'] ) );
	}
	public function boat_change_order_received_text( $str, $order ) {
		$str = 'Thank you. Your reservation has been received.';
		echo '<style>.woocommerce-order-details,.woocommerce-thankyou-order-details li.woocommerce-order-overview__order, .woocommerce-thankyou-order-details .woocommerce-order-overview__date{display: none !important;}</style>';		

		if(!empty(get_post_meta( $order->get_id(), 'date_rent', true ))){
			$new_str = $str . '<ul class="custom_received">
	    				
	    		<li class="order">Reservation number: <strong>'. $order->get_order_number().'</strong></li>	
				...
				<li class="capacity">Capacity: <strong>'.get_post_meta( $order->get_id(), 'capacity', true ).'</strong></li>
	    	</ul>';
		}
		else{
			$new_str = $str . '<ul class="custom_received">
	...
	function custom_button_text_place_order( $button_text ) {
	   $reserve_label = !empty(get_option('reserve_now_archive_label'))?get_option('reserve_now_archive_label'):'Reserve Now' ;
